...
function Health() {
  ...
  useEffect(() => {
    const FetchNews = async () => {
      const API_KEY3=import.meta.env.VITE_API_KEY
      const response = await fetch(`https://gnews.io/api/v4/top-headlines?category=health&country=in&lang=en&apikey=${API_KEY3}`)
      if (!response.ok) {
        alert("Error")
      }

...
function Sports() {
  ...
  useEffect(() => {
    const FetchNews = async () => {
      const API_KEY4=import.meta.env.VITE_API_KEY
      const response = await fetch(`https://gnews.io/api/v4/top-headlines?category=sports&country=in&lang=en&apikey=${API_KEY4}`)
      if (!response.ok) {

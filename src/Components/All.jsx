...
function All() {
  ...
  useEffect(() => {
    const FetchNews = async () => {
      ...
      const response = await fetch(`https://gnews.io/api/v4/top-headlines?category=general&country=in&lang=en&apikey=${API_KEY}`)
      if (!response.ok) {
        alert('Error')
      }
      ...
      {Loading ? (
        ...
        <div className=" m-2 absolute top-15">
          {News.articles.map((n, index) => (
            <div className="shadow-md inset-xl drop-shadow-2xl bg-slate-100  mb-4 rounded border-red-950 p-2" key={index}>
